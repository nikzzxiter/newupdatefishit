...
function AutoFishingNewMethod()
    ...
    task.spawn(function()
        ...
        print("[AutoFishingNewMethod] Started - EXTREME SPEED 5x Mode")
        
        -- Equip rod once at start - SUPER FAST
        ...
        task.wait(0.1)  -- Minimal initial wait
        ...
        while Config.AutoFishingNewMethod and RuntimeState.IsFishingNewMethod do
            local success, err = pcall(function()
                -- Wait if selling - INSTANT CHECK
                while RuntimeState.IsSelling do
                    task.wait(0.01)  -- Almost instant
                end
                
                -- Validate character - INSTANT CHECK
                if not LocalPlayer.Character or not HumanoidRootPart then
                ...
                if Remotes.ChargeRod then
                    ...
                    for attempt = 1, 2 do
                        ...
                        task.wait(0.01)  -- Almost no wait
                    end
                    
                    if not chargeSuccess then
                        error("Charge failed")
                    end
                    task.wait(0.02)  -- Reduced from 0.12 to 0.02 (83% reduction)
                end
                
                -- Step 2: Start Minigame - ULTRA FAST
                if Remotes.StartMini then
                    local startSuccess = false
                    for attempt = 1, 2 do
                        local ok = pcall(function()
                ...
                -- Step 3: Wait - EXTREME REDUCTION
                local actualDelay = math.max((Config.FishingDelay or 0.3) * 0.15, 0.02)  -- 85% reduction, minimum 0.02
                task.wait(actualDelay)
                ...
                -- MICROSCOPIC delay between cycles
                task.wait(0.000001)  -- Nearly instant
            end)
